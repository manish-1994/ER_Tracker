...
| worksheetService.ts | 44 | `.from("sheets").insert({ workbook_id, name: title })` |
| worksheetService.ts | 56 | `.from("sheets").update({ name: updates.title })` |
...
4. `frontend/src/pages/Worksheet.tsx` - 1 table reference + field mapping
...
| `worksheet_rows.*` | `records_<worksheetId>.*` |

## Verification
- ✅ Build passes: `npm run build` completes successfully
- ⚠️ Navigate to Workbooks page
- ⚠️ Click Inspect on workbook 9
- ⚠️ Verify Network tab shows 200 OK on `/rest/v1/sheets?select=*&workbook_id=eq.9`
- ⚠️ Verify Sheet Node 3/5/7/8/9 loads successfully